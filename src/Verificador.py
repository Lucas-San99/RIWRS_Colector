# ==============================================================================
# verificador.py (VERSÃO FINAL: Logs de Status em Tempo Real)
# ==============================================================================
import os
import requests
import hashlib
...
class Verificador:
    ...
    def download_url(cls, url):
        ...
        if not cls.OUTPUT_DIR_TEMP:
            logger.critical("ERRO CRÍTICO DE CONFIGURAÇÃO: OUTPUT_DIR_TEMP não configurado.")
            raise RuntimeError("OUTPUT_DIR_TEMP não configurado. Chame Verificador.set_config() primeiro.")
        ...
        try:
            ...
            if response is not None and status_code >= 400:
                # Logamos com o código de status
                logger.error(f"HTTP_ERROR: Status {status_code} ({error_message}) -> URL: {url}")
            else:
                 # Logamos como erro de rede (Timeout, Conexão, DNS)
                 logger.error(f"NETWORK_FAILURE: Status {status_code} ({error_message}) -> URL: {url}")
                 
            # Retorna a mensagem para o CSV de log
            return (url, filename, f'ERROR_{error_message}')
            
        except Exception as e:
            # Erro fatal (disco, memória, etc.)
            error_message = cls.format_error_message(e)
            logger.critical(f"FATAL_ERROR: {url} -> Falha Interna ({error_message})")
